...
if not moneytruck then
    if isRobbing and item.count > 0 and amount > 0 then
        ...
        if CopsConnected >= Config.Copsneeded then
            ...
            isRobbing = false
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = (Config.msgs.notEnoughtPolice) })
            --TriggerClientEvent('esx:notification','~r~Not Enough Police', source, r)
         end
    else
        isRobbing = false
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = (Config.msgs.notRightTool) })
    end
else
    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = (Config.msgs.robbingAlert) })
end    
end)
    ...
    if isRobbing == true then
		for i=1, #xPlayers, 1 do
            ...
            if xPlayer.job.name == 'police' then
                ...
                TriggerClientEvent('RS7x:NotifyPolice', xPlayers[i], Config.msgs.robbingAlertWithStreet ..' | ' .. street1 .. " | " .. street2 .. ' ')
			end
		end
	end
end)
