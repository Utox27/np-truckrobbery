...
AddEventHandler('RS7x:Blip', function(x,y,z)
  Blip = AddBlipForCoord(x,y,z)
    ...
    AddTextComponentString(Config.msgs.robInProgress)
...
function PlateCheck(CurPlate, CurrentPlate)
    if CurrentPlate ~= CurPlate then
        ...
        exports['pogressBar']:drawBar(3500, Config.msgs.openingDoor)
        Citizen.Wait(3500)
        looting = true
        isRobbing = true
        TriggerEvent('animation:rob')
        exports['pogressBar']:drawBar(Config.Timer * 1000, Config.msgs.takingCash)
        ...
        if not IsEntityDead(GetPlayerPed(-1)) then
            ...
            if pedSpawned == true and vehicle then
                ...
                if dstCheck < 5.0 then
                    DrawText3Ds(text.x, text.y, text.z, Config.msgs.keyRob)
            ...
            if dstCheck < 5.0 then
                if not hasRobbed then
                    if not RobbedPlates[Plate] then
                        ...
                        exports['mythic_notify']:DoHudText('error', Config.msgs.truckEmpty)
                    end
                end
            end
        end
...
function cb1(success, timeremaining)
    if success then
        ...
        exports['mythic_notify']:DoHudText('error', Config.msgs.hackFailed)
        TriggerEvent('mhacking:hide')
        TriggerServerEvent('RS7x:NotifyPolice', street1, street2, pos)
        Hacking = false
        Wait(30 * 1000) -- add a time penalty if failed, so it gives police more time to arrive // feel free to remove
        isRobbing = false
        exports['mythic_notify']:DoHudText('success', Config.msgs.hitAgain)
        TriggerServerEvent('RS7x:moneytruck_false')
    end
end
